#ifndef ICHEMIATEST_HPP_21
#define ICHEMIATEST_HPP_21
...
private:
    ...
    bool isIschemic(int idx, double x, double y, double z) {
    	...
    	if ((x - cx)*(x - cx) + (y - cy)*(y - cy) + (z - cz)*(z - cz) < 1.3 * 4444444) {
             return true;
    	}
        return false;
    }
...
public:
    CellFactory_Ichemia() : AbstractCardiacCellFactory<3>()
    {
    	...
    	region.push_back(3.7);
    	region.push_back(-16.8);
    	region.push_back(-22.8);
...
public:
    ...
    void TestIschemicHeart()
    {
        ...
        HeartConfig::Instance()->SetOutputDirectory("IschemiaOnHeart_21");
	...
	AbstractTetrahedralMesh<3,3>* p_mesh = &(bidomain_problem.rGetMesh());
        VtkMeshWriter<3,3> vtk_writer("./ISHEMIA_21/", "init_mesh_1", false);
        vtk_writer.WriteFilesUsingMesh(*p_mesh);

        VtkMeshWriter<3,3> vtk_writer2("./ISHEMIA_21/", "init_mesh_2", false);
