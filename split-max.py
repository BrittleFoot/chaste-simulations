...
def plot_by_electrode(file_mask):
    ...
    fig, axes = plt.subplots(1, 1, sharex='all', figsize=(12, 5))
    ...
    for key, axe in zip(keys, list(np.nditer(axes, flags=['refs_ok']))):
        ...
        axe.set_title("Одномерная модель")
        ...
        for file, (x, y) in charts[key]:
            ...
            if a == 1:
                axe.plot(x, y, color=(0, 0, 1, 1), label='Без ишемии')

            if a == 0.1:
                axe.plot(x, y, color=(1, 0, 0, 1), label='С ишемией')
        axe.legend()
    plt.show()
