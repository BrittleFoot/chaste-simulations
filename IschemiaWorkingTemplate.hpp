...
private:
    ...
    bool isIschemic(int idx, double x, double y, double z) {
    	...
    	if ((x - cx)*(x - cx) + (y - cy)*(y - cy) + (z - cz)*(z - cz) < 44444444 * 44444444) {
             return true;
    	}
        return false;
    }
...
public:
    CellFactory_Ichemia() : AbstractCardiacCellFactory<3>()
    {
    	...
    	region.push_back(11111111);
    	region.push_back(22222222);
    	region.push_back(33333333);
...
public:
	...
	AbstractTetrahedralMesh<3,3>* p_mesh = &(bidomain_problem.rGetMesh());
        VtkMeshWriter<3,3> vtk_writer("./ISHEMIA_00000000/", "init_mesh_1", false);
        vtk_writer.WriteFilesUsingMesh(*p_mesh);

        VtkMeshWriter<3,3> vtk_writer2("./ISHEMIA_00000000/", "init_mesh_2", false);
